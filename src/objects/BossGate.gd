...
func _process(delta: float) -> void:
	...
	if not player.position.y - position.y <= y_limits[1] and player.position.y - position.y >= y_limits[0]:
...
func check_for_player():
	...
	if player.position.y > position.y + 74: return
	elif player.position.y < position.y + 10: return
	going_through = true
...
func animation_doing():
	...
	if going_through_timer == start_time:
		...
		$Camera.position.y += 32 if $Camera.get_camera_position().y < player_cam.get_camera_position().y else\
			-32 if $Camera.get_camera_position().y > player_cam.get_camera_position().y else 0
		$Camera.current = true
		if abs($Camera.get_camera_position().y - player_cam.get_camera_position().y) < 32:
			player_cam.current = true
